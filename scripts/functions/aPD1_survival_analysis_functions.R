...
optCutpointContinuous <- function(DF,timeVar,eventVar,varVector,filterVar=NULL, filterVarSelected=NULL, gatherTable=FALSE){
  ...
  if(!isTRUE(gatherTable)){
    if (!is.null(filterVar)){
      ...
      DF <- DF %>% dplyr::filter(!!filterVar.un %in% dplyr::filterVarSelected) %>% droplevels()
      DF
    }
  }else{
    if (!is.null(filterVar)){
...
#     summarize(no_rows=length(!!facetVariable.un)) %>% dplyr::mutate(comboVar=paste0(!!facetVariable.un, "-", corVarx))
#   # tissue_dataPoints
...
fit_all <- function(selectedcancer,tcgaDF,survHRdf,cutPointsTable,covName,
  ...
  tcgaDF_sign <- dplyr::filter(tcgaDF, !!facetVariable.un %in% dplyr::selectedcancer) %>% dplyr::filter(complete.cases(!!event.un,!!endopoint.un)) %>% droplevels()
...
runMultiSurvival_TCGA <- function(DF,var,varSelection,varSelected,survType,sampleID,varPlotName,printP=FALSE,saveP = FALSE,savePath,onlySign=TRUE,selectFacets=NULL){
  ...
  if(isTRUE(onlySign)){
    ...
    if(!is.null(selectFacets)){
      ...
      hrDF.sign <- res.cat2$hrDF %>% as.data.frame() %>% dplyr::filter(!!varSelected.un %in% dplyr::selectFacets) %>% droplevels()
...
# t <-dataDF %>% dplyr::filter(complete.cases(OS)) %>% droplevels()
multiUniSurvival <- function(dataDF,surv_time,surv_status,surv_time_label, features,labels, removeMis=FALSE, 
                             filterCohort=FALSE, filterVar=NULL,cohortSelected=NULL, 
                             categTrans=NULL, transBy="median",multi=FALSE,scale=FALSE,alreadyCat=NULL){
  #categTrans if there are continuous variables to be transformed to categorical.
  # #
  # dataDF <-immdata.TcrBcr.full.red.sub
  # surv_time <- OS_time
  # surv_status <- OS_status
  # surv_time_label <- "OS"
  # features <- c(featuresIn,"gender")
  # labels <- finallabels
  # removeMis=FALSE
  # filterCohort=TRUE
  # filterVar="tissue"
  # cohortSelected="gastric"
  # categTrans=featuresIn
  # transBy="median"
  # multi=FALSE
  # alreadyCat=NULL
  ...
  surv_status.un <- as.name(surv_status)
  surv_time.un <- as.name(surv_time)
  
  if(isTRUE(filterCohort)){
    cat('\n')  
    cat('\n') 
    cat("#### ", cohortSelected, "\n") 
    cat('\n') 
  ...
  # check if I need to filter Cohort
  if(isTRUE(filterCohort)){
    dataDF <- dataDF %>% dplyr::select(all_of(c(features,surv_time,surv_status,filterVar))) %>% droplevels() %>% distinct()
    
    print(paste0(">> Subsetting data to ", cohortSelected, "\n"))
    filterVar.un <- as.name(filterVar)
    dataDF.filt <- dataDF %>% dplyr::filter(!!filterVar.un %in% cohortSelected) %>% distinct() %>% droplevels()
    # print(paste0(">>Total ", nrow(dataDF.filt)," patients included<<", "\n"))
    dataDF <- dataDF.filt
    
  }else{
    dataDF <- dataDF %>% dplyr::select(all_of(c(features,surv_time,surv_status))) %>% droplevels() %>% distinct()
    
  }
  
  # Here I have to check is some features are NA for all available data. If so I need to exclude this variable from the analysis
  NaCols <-as.vector(which(sapply(dataDF, function(x)all(is.na(x)))))
  NaColslabels <-as.vector(names(which(sapply(dataDF, function(x)all(is.na(x))))))
  if(length(NaCols)>=1){
    print(paste0(">>Column/Features with all Nas were found, we need to exclude them from the analysis: ", "\n"))
    print(colnames(dataDF)[NaCols])
    dataDF<- dataDF[,-NaCols]
    # Now fix labels used in survival analysis
    labels <-labels[-which(names(labels) %in% NaColslabels)]
    features <- features[-which(features %in% NaColslabels)]
    categTrans <- categTrans[-which(categTrans %in% NaColslabels)]
    # finallabels
  }
  
  # check if I remove missing
  if(isTRUE(removeMis)){
    dataDF.filt <- dataDF %>% dplyr::select(all_of(c(features,surv_time,surv_status))) %>% dplyr::filter(complete.cases(.)) %>% droplevels()
    print(paste0(nrow(dataDF)-nrow(dataDF.filt)," patients with missing data across data", "\n"))
    dataDF <- dataDF.filt
    print(paste0(">>Final total ", nrow(dataDF)," patients included<<", "\n"))
    
  }else{
    dataDF.filt <- dataDF %>% dplyr::select(all_of(c(features,surv_time,surv_status))) %>% dplyr::filter(complete.cases(.)) %>% droplevels()
    # print(paste0(nrow(dataDF)-nrow(dataDF.filt)," patients with missing data across data", "\n"))
    # dataDF <- dataDF.filt
    print(paste0(">>Final total if we removed missing ", nrow(dataDF.filt)," patients included<<", "\n"))
    print(paste0(">>Final total ", nrow(dataDF.filt)," patients included<<", "\n"))
    
  }
  
  ## A. The problem of no convergence can occur when filtered (no missing) data in multi includes only ONE EVENT (e.g. only 0, or very few 1s).
  ## B. no matter if I remove missing or not, the model does it by itself, so when very few data are included, especially in multivariate analysis
  # I will set a minimum of 15 observations to be available for multivariate analysis
  # if(isTRUE(multi)){
  #   dataDF.filt <- dataDF %>% dplyr::select(all_of(c(features,surv_time,surv_status))) %>% dplyr::filter(complete.cases(.)) %>% droplevels()
  #   if(nrow(dataDF.filt)<= 10){
  #     print(paste0(">>Final total ", nrow(dataDF.filt)," patients included<<"))
  #     print(paste0(">>NOT ENOUGH DATA FOR SURVIVAL ANALYSIS, skipping...<<"))
  #     return()
  #   }
  #   # if()
  # }
  ...
  # Here find a way to transform cont variables to catgorical:FOR NOW IT WORKS IF ALL VARS ARE TRANSFORMED
  ## IMPORTANT: if we do not remove missing data, at this point the calculation of the median is including all available for the featuer,
  # but in the end some values might not have equivalent for OS, OS.time, so the final median will be different
  if(!is.null(categTrans)){
    #   print(paste0(">>RUNNING CATEGORICAL<<", "\n"))
    #   if(isTRUE(multi)){
    #     if(transBy=="median"){
    #       categ <- function(x, na.rm = FALSE) (ifelse(x <= median(x,na.rm = na.rm), "low","high"))
    #       
    #     }else{
    #       categ <- function(x, na.rm = FALSE) (ifelse(x <= mean(x,na.rm = na.rm), "low","high"))
    #     }
    #     factoLevel <- function(x) (factor(x,levels=c("low","high")))
    #     dataDF <- dataDF %>% dplyr::mutate(across(features, ~ categ(.x, na.rm = TRUE)))
    #     dataDF <- dataDF %>% dplyr::mutate(across(all_of(features), ~ factoLevel(.x)))
    #   }else{
    #     # dataDF <- dataDF %>% mutate_at(all_of(featuresIn), as.factor)
    #     #########################
    #     # ALTERNATIVE SOLUTION ##
    #     #########################
    #     
    #     # Find Medians across all features
    #     findMedian <- function(df,selColumn,fixedColumn){
    #       # df <- df
    #       # selColumn <-"x1"
    #       # fixedColumn <- "x2"
    #       
    #       selColumn.un <- as.name(selColumn)
    #       fixedColumn.un <- as.name(fixedColumn)
    #       
    #       # I want to extract median only
    #       md <-df %>% dplyr::select(!!selColumn.un,!!fixedColumn.un) %>% dplyr::filter(complete.cases(.)) %>% pull(!!selColumn.un) %>% median()
    #       md
    #     }
    #     mds<-sapply(features, function(x) findMedian(dataDF,x,surv_status), simplify = TRUE)
    #     
    #     # Now make columns categorical based on the medians
    #     categ2 <- function(x,md) (ifelse(x < md, "low","high"))
    #     applyMedian <- function(dataDF,med,selColumn){
    #       # dataDF<- dataDF %>% as.data.frame()
    #       # med <- mds[1]
    #       # selColumn <-features[1]
    #       dataDF[[selColumn]] <<- categ2(dataDF[[selColumn]],med)
    #     }
    #     updateCat<- sapply(1:length(features), function(x) applyMedian(dataDF,mds[x],features[x]))
    #     
    #     factoLevel <- function(x) (factor(x,levels=c("low","high")))
    #     dataDF <- dataDF %>% dplyr::mutate(across(all_of(features), ~ factoLevel(.x)))
    #   }
    ...
    mds<-sapply(categTrans, function(x) findMedian(dataDF,x,surv_status), simplify = TRUE)
    print(mds)
    # Now make columns categorical based on the medians
    categ2 <- function(x,md) (ifelse(x < md, "low","high"))
    applyMedian <- function(dataDF,med,selColumn){
    ...
    updateCat<- sapply(1:length(categTrans), function(x) applyMedian(dataDF,mds[x],categTrans[x]))
    
    factoLevel <- function(x) (factor(x,levels=c("low","high")))
    dataDF <- dataDF %>% dplyr::mutate(across(all_of(categTrans), ~ factoLevel(.x)))
    
    
    # Make the names
    featNames<-c()
    for (i in 1:length(categTrans)){
      featNames <-c(featNames,c(categTrans[i], paste0(categTrans[i],":high"), paste0(categTrans[i],":low")))
      featNames
    }
    # featNames
    
    if(!is.null(alreadyCat)){
      idx <- which(names(labels) %in% alreadyCat)
      keepLab <-labels[idx]
      keepFeat <-features[idx]
      # Featnames
      extraFeatNames<-as.vector(unlist(sapply(keepFeat, function(x) c(x,paste0(x,": ",levels(dataDF[[x]]))),simplify = FALSE)))
      # Labels
      labels <- labels[-idx]
      extraLabels<-as.vector(unlist(sapply(keepFeat, function(x) c(keepLab[x],paste0(keepLab[x],": ",levels(dataDF[[x]]))),simplify = FALSE)))
    }
    
    featLabels <- c()
    
    for (i in 1:length(labels)){
      featLabels <-c(featLabels,c(labels[[i]], paste0(labels[[i]],": high"), paste0(labels[[i]],": low")))
      featLabels
    }
    # featLabels
    if(!is.null(alreadyCat)){
      featNames<- c(featNames,extraFeatNames)
      featLabels<- c(featLabels,extraLabels)
    }
    names(featLabels) <- featNames
    labels<-featLabels
    
  }
  
  # Check if dataset includes only one event in final data
  # WITH MISSINS
  if(length(unique(dataDF$OS))==1){
    print(paste0(">>Only data for one event available, skipping...<<", "\n"))
    return()
  }
  
  # WITHOUT MISSING
  if(length(unique(dataDF %>% dplyr::select(all_of(c(features,surv_time,surv_status))) %>% dplyr::filter(complete.cases(.)) %>% droplevels() %>% pull(OS)))==1){
    print(paste0(">>Only data for one event available, skipping...<<", "\n"))
    return()
  }
  ...
  # When a feature is only one category - does it affect both uni and multi?
  
  ## SET CRITERIUM: minimum 8 events per covariate
  # we check it here, depending on univariate or multivariate
  # If univariate we divide by 1, or if categorical we need to count levels
  # If multivariate, we divide by total number of covariates
  # and we need to calculate the correct number, considering categorical variables: n(levels)-1
  if(!isTRUE(multi)){## UNIVARIATE
    # First Calculate covariates,
    num_cols <- unlist(lapply(dataDF %>% dplyr::select(all_of(features)), is.numeric))
    categ_cols <- unlist(lapply(dataDF %>% dplyr::select(all_of(features)), is.factor))
    # Calculate number of actual covariates from categ factors
    if(length(which(categ_cols))>0){# IF THERE ARE CATEGORICAL
      categCov <-sapply(names(which(categ_cols==TRUE)), function(x) length(levels(dataDF[[x]]))-1, simplify=FALSE)
      n.cat <- unlist(categCov ) 
      # For univariate we need to analyze them together, if we have e.g. a covariate with many levels
      # and the data are not enough we have to remove that feature from univariate analysis
      n.events <- sapply(names(n.cat), function(x) dataDF %>% dplyr::select(all_of(c(surv_time,surv_status,x))) %>% dplyr::filter(complete.cases(.)) %>% 
                           dplyr::count(!!surv_status.un) %>% dplyr::filter(!!surv_status.un==1) %>% pull(n), simplify=FALSE)
      n.events <- unlist(n.events)
      n.ePc.cat <- n.events[names(n.cat)]/n.cat
      n.categFail <- names(which(n.ePc.cat <=7))
      print("Events per categorical covariate:")
      print(n.ePc.cat)
      
      if(length(n.categFail)>=1){# if some vars failed, remove them
        print("The following categorical variables have less than 7 events, they are removed from analysis")
        dataDF <- dataDF %>% dplyr::select(-all_of(n.categFail)) %>% droplevels()
      }
    }
    if(length(which(num_cols))>0){# IF THERE ARE NUMERICAL
      # numCov <-sapply(names(which(num_cols==TRUE)), function(x) length(levels(dataDF[[x]]))-1, simplify=FALSE)
      # n.num <- unlist(categCov ) 
      n.events <- sapply(names(which(num_cols==TRUE)), function(x) dataDF %>% dplyr::select(all_of(c(surv_time,surv_status,x))) %>% dplyr::filter(complete.cases(.)) %>% 
                           dplyr::count(!!surv_status.un) %>% dplyr::filter(!!surv_status.un==1) %>% pull(n), simplify=FALSE)
      n.events <- unlist(n.events)
      
      n.ePc.num <- n.events[names(which(num_cols==TRUE))]/1
      n.numFail <- names(which(n.ePc.num <=7))
      
      print("Events per numerical covariate:")
      print(n.ePc.num)
      
      if(length(n.numFail)>=1){# if some vars failed, remove them
        print("The following numerical variables have less than 7 events, they are removed from analysis")
        dataDF <- dataDF %>% dplyr::select(-all_of(n.numFail)) %>% droplevels()
      }
    }
    
    # Now we need to evaluate if there are any of the features in the dataDF, to be able to do the survival analysis.
    if(!isTRUE(any(features %in% colnames(dataDF)))){
      print(paste0(">>Less than 7 events per covariate for all covariates, skipping...<<<<", "\n"))
      return()
    }
    ...
    # First Calculate covariates,
    num_cols <- unlist(lapply(dataDF %>% dplyr::select(all_of(features)), is.numeric))
    categ_cols <- unlist(lapply(dataDF %>% dplyr::select(all_of(features)), is.factor))#[,features]
    n.num <- length(which(num_cols==TRUE))
    # n.cat <- length(which(categ_cols==TRUE))
    # names(which(categ_cols==TRUE))
    
    categCov <-sapply(names(which(categ_cols==TRUE)), function(x) length(levels(dataDF[[x]]))-1, simplify=FALSE)
    n.cat <- unlist(categCov ) %>% sum()
    n.cov <- n.num + n.cat
    
    # Now calculate number of events per covariate
    
    n.events <- dataDF %>% dplyr::select(all_of(c(features,surv_time,surv_status))) %>% 
                dplyr::filter(complete.cases(.)) %>% 
                droplevels() %>% 
                dplyr::count(!!surv_status.un) %>% 
                dplyr::filter(!!surv_status.un==1) %>% pull(n)
    n.ePc <- n.events/n.cov
    
    print("Events per covariate:")
    print(n.ePc)
    
    if(n.ePc <=7){
      print(paste0(">>Less than 7 events per covariate, skipping...<<", "\n"))
      return()
    }
  }
  
  # Run uni/multivariate analysis
  if(isTRUE(multi)){
    print(paste0(">>MULTIVARIATE ANALYSIS<<", "\n"))
    
    res <-myTryCatch(analyse_multivariate(dataDF,
                                          c(surv_time,surv_status),
                                          covariates = features, # covariates expects a list
                                          covariate_name_dict = labels,
                                          covariate_label_dict = labels)
    )
    
    if(isTRUE(grepl("Ran out of iterations and did not converge", res$warning))){
      print(paste0(">>Survival model ran out of iterations & did not converge, skipping...<<", "\n"))
      
      res.stack.df <- res$value$summaryAsFrame
      
    }else{
      res <- res$value
      print(paste0(">>COLLECTING MULTIVARIATE RESULTS<<", "\n"))
      
      results.multi.coxph <- res$coxph %>% tbl_regression(exponentiate = TRUE,
      ...
      res.stack <- results.multi.coxph
      res.stack.df <- res$summaryAsFrame
      
    }
  }else{
    print(paste0(">>UNIVARIATE ANALYSIS<<", "\n"))
    res <-map(features, function(by){analyse_multivariate(dataDF,
                                                          c(surv_time,surv_status),
    ...
    print(paste0(">>COLLECTING UNIVARIATE RESULTS<<", "\n"))
    ## For univariate
    results.uni.coxph <-lapply(res, function(x) x[["coxph"]])
    gtables.uni.coxph <-lapply(1:length(results.uni.coxph), function(x) tbl_regression(results.uni.coxph[[x]],exponentiate = TRUE,
                                                                                       pvalue_fun = fmt_pvalue_with_stars
                                                                                       #,label=var.names[[x]]
    )%>%
      modify_footnote(p.value ~ "*p<0.05; **p<0.01; ***p<0.001"))
    res.stack <- tbl_stack(gtables.uni.coxph)
    res.stack.df <- ldply(sapply(res, function(x) x$summaryAsFrame, simplify = FALSE)) %>% as.data.frame()
  }
  
  
  
  if(isTRUE(grepl("Ran out of iterations and did not converge", res$warning))){
    res=list(res=res$value,
             res.tbl = NULL,
             res.df = res.stack.df,
             forestP=NULL)
  }else{
    ## FOREST PLOT ##
    forestPlot <- res %>%forest_plot(
      factor_labeller = labels,
      ...
      # HR_x_breaks = c(0.1,0.25,0.5,0.75,1,2,4,10,15,25)
...
multiSurv.BackIndepBoot <- function(dataDF,surv_time,surv_status,surv_time_label, features,labels, removeMis=FALSE, 
                                    filterCohort=FALSE, filterVar=NULL,cohortSelected=NULL, 
                                    categTrans=NULL, transBy="median",multi=FALSE,scale=FALSE,alreadyCat=NULL,bootFreq=0.2){
  #categTrans if there are continuous variables to be transformed to categorical.
  # # #
  # dataDF <-immdata.TcrBcr.full.red.sub
  # surv_time <- OS_time
  ...
  if(length(NaCols)>=1){
    ...
    # Now fix labels used in survival analysis
    labels <-labels[-which(names(labels) %in% NaColslabels)]
    orig.labels <- orig.labels[-which(names(orig.labels) %in% NaColslabels)]
  ...
  ## BOOTSTRAPPING to make backward selection more valid
  # Run backwards selection, using AIC criterion
  
  backSel_function <- function(data,inters=FALSE,featsVec){
    # data <- dataDF
    # i <- sample(1:nrow(data),130)
    d2 <- data 
    
    # With first order interactions
    if(isTRUE(inters)){
      f <- selectCox(as.formula(paste0("Surv(",surv_time,",",surv_status,")","~ (",paste(featsVec, collapse = " + "),")^2")) ,
                     data=d2 %>% dplyr::filter(complete.cases(.)),rule="aic")
      
    }else{
      f <- selectCox(as.formula(paste0("Surv(",surv_time,",",surv_status,")","~ ",paste(featsVec, collapse = " + "))) ,
                     data=d2 %>% dplyr::filter(complete.cases(.)),rule="aic")
    }
    #     
    ...
    return(paste0(f$In,collapse = ", "))
  }
  
  
  # ### BACKWARD SELECTION WITH BOOTSTRAPPING
  # ## NO INTERACTIONS
  # print(paste0(">>Calculing bootstrap inclusion frequences, using backward selection.<<", "\n"))
  # set.seed(3456)
  # boot_backSel.noInter <- mosaic::do(10000, parallel=TRUE)*backSel_function(data=mosaic::resample(dataDF), inters = FALSE, featsVec = features)
  # # Calculate Frequences 
  # backSel.noInter <-boot_backSel.noInter$backSel_function %>%
  #   strsplit(split=",") %>% unlist %>% 
  #   # remove instances of characters you don't care about
  #   `[`(!. %in% c("", " ", ".", ",")) %>% str_replace_all(" ","") %>%
  #   # make a frequency table of the characters
  #   table 
  # 
  # backSel.noInter.freqs <- backSel.noInter/10000
  # # backSel.noInter.bar <- barplot(backSel.noInter)
  
  
  # ## INTERACTIONS INCLUDED
 # Just using bootstrapping for independence
  # Try to extract only hazard ratio, not pvalues for significance
  
  multi_function <- function(data,featsVec){
    # data <- dataDF
    # featsVec <- featuresIn
    # i <- sample(1:nrow(data),130)
    d2 <- data 
    d2 <-d2 %>% dplyr::select(all_of(c(featsVec,surv_time,surv_status))) %>% dplyr::filter(complete.cases(.)) %>% droplevels()
    # With first order interactions
    
    f <- eval(parse(text=paste0("coxph(Surv(",surv_time,",",surv_status,")","~ ",paste(featsVec, collapse = " + "),",data=d2)")))
    s <- summary(f)
    #   # Hazard ratio
  ...
  # ## BOOTSTRAPPED MODEL
  # print(paste0(">>Calculing bootstrap hazard ratios and CIs...<<", "\n"))
  # set.seed(3456)
  # boot_indep.HR <- mosaic::do(10000, parallel=TRUE)*multi_function(data=mosaic::resample(dataDF),featsVec = features)
  # boot_indep.HR.CI<-confint(boot_indep.HR)
  # boot_indep.HR.CI <- boot_indep.HR.CI %>% column_to_rownames(var="name") %>% dplyr::select(estimate, lower, upper)
  # # P-value calculation
  # # u.l <- (log(u)-log(l))/2
  # # sterr <- u.l / 1.96
  # # z <- log(hr)/sterr
  # # pval <- 2*pnorm(-abs(z))
  # # 
  # # pval <- 2*pnorm(-abs( log(hr)/(((log(u)-log(l))/2)/1.96) ))
  # ## BELOW IS WRONG-READ EXPLANATION ABOUT P VALUES FROM BOOTSTRAP
  # #You already get p<0.05/p>0.05 based on whether the confidence interval does not/does include 0. 
  # # Any more detail is not possible since the distribution of the parameter from the bootstrap is not 
  # # parametric (and thus you can not get a probability that the value is 0)
  # u.l <- (log(boot_indep.HR.CI$upper)-log(boot_indep.HR.CI$lower))/2
  # sterr <- u.l / 1.96
  # z <- log(boot_indep.HR.CI$estimate)/sterr
  # pval <- 2*pnorm(-abs(z))
  # boot_indep.HR.CI$p <- pval
  # 
  # #exp(-0.717*(abs((log(boot_indep.HR.CI$estimate))/((log(boot_indep.HR.CI$upper) - log(boot_indep.HR.CI$lower))/(2*1.96)))) - 0.416*((abs((log(boot_indep.HR.CI$estimate))/((log(boot_indep.HR.CI$upper) - log(boot_indep.HR.CI$lower))/(2*1.96))))^2))
  # colnames(boot_indep.HR.CI) <-c("HR","Lower_CI","Upper_CI","p")
  
  
  ## SIMPLE NON BOOTSTRAPPED MODEL
  model <- coxph(formula = as.formula(paste0("Surv(",surv_time,",",surv_status,")","~ ",paste(features, collapse = " + "))), data = dataDF, model=TRUE)
  s<-summary(model)
  m.ci <- s$conf.int[,c(1,3,4)]
  m.ci <- m.ci %>% as.data.frame() %>% rownames_to_column(var="factor.id") %>% add_column(factor.name = orig.labels, .after = "factor.id")
  m.hr.final <- cbind(m.ci,s$coefficients[,5])
  colnames(m.hr.final)[3:6] <- c("HR","Lower_CI","Upper_CI","p")
  
  print(paste0(">>Calculing bootstrapped pvals &  CIs...<<", "\n"))
  
  ## BOOTSTRAPPED CIs, non bootstrapped model
  # Get bootstrap confidence intervals, and bootstrap p-values for the model coefficients, also adjusted p-values
  modelBoot <-censboot_summary(model, R = 10000,adjust.method="BH", coef = "exp")
  modelBoot <- modelBoot %>% as.data.frame() %>% rownames_to_column(var="factor.id") %>% add_column(factor.name = orig.labels, .after = "factor.id")
  colnames(modelBoot)[3:7] <- c("HR","Lower_CI","Upper_CI","p","p.adj")
  
  # # Now do the same for multi, with vars selected by bootstrapping frequency
  # print(paste0(">>Re-Calculing bootstrap hazard ratios and CIs for most frequent vars...<<", "\n"))
  # bootFreq.vars <-  which(backSel.noInter.freqs>=bootFreq) %>% names()
  
  # if(length(bootFreq.vars)==0){
  #   # very low frequencies
  #   print(paste0(">>None frequent vars...skipping<<", "\n"))
  #   
  #   #####
  #   res=list(simple = m.hr.final,
  #            # bootSel=backSel.noInter.freqs,
  #            # bootModel = boot_indep.HR.CI,
  #            bootCIpval = modelBoot)
  # }else{
    # set.seed(3456)
    # boot_indep.HR2 <- mosaic::do(10000, parallel=TRUE)*multi_function(data=mosaic::resample(dataDF),featsVec = bootFreq.vars)
    # boot_indep.HR.CI2<-confint(boot_indep.HR2)
    # boot_indep.HR.CI2 <- boot_indep.HR.CI2 %>% column_to_rownames(var="name") %>% dplyr::select(estimate, lower, upper)
    # #boot_indep.HR.CI2$p <- exp(-0.717*(abs((log(boot_indep.HR.CI2$estimate))/((log(boot_indep.HR.CI2$upper) - log(boot_indep.HR.CI2$lower))/(2*1.96)))) - 0.416*((abs((log(boot_indep.HR.CI2$estimate))/((log(boot_indep.HR.CI2$upper) - log(boot_indep.HR.CI2$lower))/(2*1.96))))^2))
    # u.l <- (log(boot_indep.HR.CI2$upper)-log(boot_indep.HR.CI2$lower))/2
    # sterr <- u.l / 1.96
    # z <- log(boot_indep.HR.CI2$estimate)/sterr
    # pval <- 2*pnorm(-abs(z))
    # boot_indep.HR.CI2$p <- pval
    # colnames(boot_indep.HR.CI2) <-c("HR","Lower_CI","Upper_CI","p")
    
    #####
    res=list(simple = m.hr.final,
             # bootSel=backSel.noInter.freqs,
             # bootModel = boot_indep.HR.CI,
             # bootRedModel = boot_indep.HR.CI2,
             bootCIpval = modelBoot)
  # }
...
multiSurv.BootCIpval <- function(dataDF,surv_time,surv_status,surv_time_label, features,labels, removeMis=FALSE, 
                                    filterCohort=FALSE, filterVar=NULL,cohortSelected=NULL, 
                                    categTrans=NULL, transBy="median",multi=FALSE,scale=FALSE,alreadyCat=NULL){
  #categTrans if there are continuous variables to be transformed to categorical.
  # # #
  ...
  # surv_status <- OS_status
  # surv_time_label <- "OS"
  # features <- featuresIn[1]
  # labels <- finallabels[1]
  # removeMis=FALSE
  # filterCohort=TRUE
  # filterVar="tissue"
  # cohortSelected="melanoma"
  # categTrans=featuresIn[1]
  # transBy="median"
  # multi=FALSE
  # alreadyCat=NULL
  # bootFreq=0.2
  ...
  # print(paste0(">>Calculing bootstrap hazard ratios and CIs...<<", "\n"))
  # set.seed(3456)
  # boot_indep.HR <- do(10000, parallel=TRUE)*multi_function(data=mosaic::resample(dataDF),featsVec = features)
  # boot_indep.HR.CI<-confint(boot_indep.HR)
  # boot_indep.HR.CI <- boot_indep.HR.CI %>% column_to_rownames(var="name") %>% dplyr::select(estimate, lower, upper)
  # boot_indep.HR.CI$p <- exp(-0.717*(abs((log(boot_indep.HR.CI$estimate))/((log(boot_indep.HR.CI$upper) - log(boot_indep.HR.CI$lower))/(2*1.96)))) - 0.416*((abs((log(boot_indep.HR.CI$estimate))/((log(boot_indep.HR.CI$upper) - log(boot_indep.HR.CI$lower))/(2*1.96))))^2))
  # colnames(boot_indep.HR.CI) <-c("HR","Lower_CI","Upper_CI","p")
  # SIMPLE NON BOOTSTRAPPED MODEL
  model <- coxph(formula = as.formula(paste0("Surv(",surv_time,",",surv_status,")","~ ",paste(features, collapse = " + "))), data = dataDF, model=TRUE)
  s<-summary(model)
 
  if(!isTRUE(multi)){ # IF UNIVARIATE NEEDS TRANSFORMING HERE
    m.ci <- as.data.frame(s$conf.int)
    m.ci <- m.ci[,c(1,3,4)]
    m.ci <- m.ci %>% as.data.frame() %>% rownames_to_column(var="factor.id") %>% add_column(factor.name = orig.labels, .after = "factor.id")
  }else{
    m.ci <- s$conf.int[,c(1,3,4)]
  ...
  print(paste0(">>Calculing bootstrapped pvals &  CIs...<<", "\n"))
  # Get bootstrap confidence intervals, and bootstrap p-values for the model coefficients
  modelBoot <-censboot_summary(model, R = 10000,adjust.method="BH", coef = "exp")
  modelBoot <- modelBoot %>% as.data.frame() %>% rownames_to_column(var="factor.id") %>% add_column(factor.name = orig.labels, .after = "factor.id")
  colnames(modelBoot)[3:6] <- c("HR","Lower_CI","Upper_CI","p","p.adj")
  ...
  model <- survreg(formula = Surv(time, status) ~ sex, data = lung,
...
runUniSurvival_Singlevariable_KMplot_aPD1 <- function(DF,var,varSelection,varSelected,survType,sampleID,varPlotName,printP=FALSE,saveP = FALSE,savePath,onlySign=TRUE,selectFacets=NULL){
  # DF =immdata.TcrBcr.full.red.sub
  # var = featuresIn[2]
  # varSelection =tissues
  # varSelected = "tissue"
  # survType = "OS"
  # sampleID = "run_accession"
  # varPlotName = featuresInLabels[2]
  # printP=TRUE
  # saveP = TRUE
  # onlySign=FALSE
  # selectFacets=tissues
  ...
  if(isTRUE(saveP)){
    res = list(forestCont = res.cont$forestplot,
               ...
               hrDF = res.cat2$hrDF)
    # print(paste0(projectRDataPath, paste0("tcga.SURV","_",survType,"_",var),Rdata.suffix))
    save(res, file = paste0(savePath, paste0("aPD1.SURV","_",survType,"_",var),Rdata.suffix))
  }
