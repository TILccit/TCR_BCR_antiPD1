...
get_tissueDataNumb <- function(tcgaDF, numb_DataPoints_df, SampleCol,corVarx,corVary,facetVariable){
  ...
  # tissue_dataPoints
  # tissues_fewData <- tissue_dataPoints %>% filter(no_rows<10) %>% select(!!facetVariable.un) %>% pull(!!facetVariable.un) %>% droplevels()
  tissue_dataPoints
...
fit_all <- function(selectedcancer,tcgaDF,survHRdf,cutPointsTable,covName,
  ...
  if(cutVar==0){
    ...
    tcgaDF_sign2 <- tcgaDF_sign %>% mutate(!!stratVariable.un := ifelse(!!predictor.un >= cutVar,StratLevel1,StratLevel2)) %>%
      dplyr::mutate(!!stratVariable:=factor(!!stratVariable.un)) %>% 
      dplyr::filter(complete.cases(!!event.un,!!endopoint.un))
  ...
  if (getLegend==TRUE){
    ...
    survPlot
    legend <- cowplot::get_legend(survPlot$plot + theme(legend.position = "top"))
    legend
  }else{
    ...
    # hr <-survHRdf %>% filter(!!facetVariable.un==selectedcancer) %>% filter(covariate==paste0(covName,StratLevel2)) %>% pull(HR)
    # ci.l <-survHRdf %>% filter(!!facetVariable.un==selectedcancer) %>% filter(covariate==paste0(covName,StratLevel2)) %>% pull(HR_lower)
    # ci.u <-survHRdf %>% filter(!!facetVariable.un==selectedcancer) %>% filter(covariate==paste0(covName,StratLevel2)) %>% pull(HR_upper)
    # ci <- c(ci.l,ci.u)
    # ## logrankp <- round(survpval$pval,5)
    # # logrankp <- formatC(survpval$pval,format = "e", digits = 3)
    # logrankp <- format(survHRdf %>% filter(!!facetVariable.un==selectedcancer) %>% filter(covariate==paste0(covName,StratLevel2)) %>% pull(logrankP),format = "e", digits = 3)
    # 
    if(covType=="cont"){
...
runUniSurvival_Singlevariable_KMplot_TCGA <- function(DF,var,varSelection,varSelected,survType,sampleID,varPlotName,printP=FALSE,saveP = FALSE,savePath,onlySign=TRUE,selectFacets=NULL){
...
multiUniSurvival <- function(dataDF,surv_time,surv_status,surv_time_label, features,labels, removeMis=FALSE, 
  ...
  if(!isTRUE(multi)){## UNIVARIATE
    ...
    num_cols <- unlist(lapply(dataDF[,features], is.numeric))
    categ_cols <- unlist(lapply(dataDF[,features], is.factor))
    ...
    # First Calculate covariates,
    num_cols <- unlist(lapply(dataDF[,features], is.numeric))
    categ_cols <- unlist(lapply(dataDF[,features], is.factor))#[,features]
...
multiSurv.BackIndepBoot <- function(dataDF,surv_time,surv_status,surv_time_label, features,labels, removeMis=FALSE, 
  ...
  # u.l <- (log(m.hr.final$Upper_CI)-log(m.hr.final$Lower_CI))/2
  # sterr <- u.l / 1.96
  # z <- log(m.hr.final$HR)/sterr
  # pval <- 2*pnorm(-abs(z))
