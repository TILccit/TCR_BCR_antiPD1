...
get_tissueDataNumb <- function(tcgaDF, numb_DataPoints_df, SampleCol,corVarx,corVary,facetVariable){
  # dataDF, survivalPointsNumb, sampleVar,paste0(survivalVar,".time"),mainVar,filterVar
  # tcgaDF <-dataDF
  # numb_DataPoints_df <-survivalPointsNumb
  # SampleCol <-sampleVar
  # corVarx <-paste0(survivalVar,".time")
  # corVary <-mainVar
  ...
  tissue_dataPoints <- tcgaDF.clean  %>% dplyr::group_by(!!facetVariable.un) %>% 
    distinct(!!SampleCol.un,.keep_all = TRUE) %>%
...
cox_hazard <- function(DF, selectedcancer, SampleCol, endopoint, event,facetVariable,predictor,hrDF,cutPointsTable=NULL,signature=NULL, 
  ...
  preNumb <-DF.sub %>% dplyr::select(predictor) %>% dplyr::filter(complete.cases(.)) %>% nrow()
  if(preNumb==0){
    print("No data for this feature!")
    return()
  }
  if(isTRUE(dichot)){
    ...
    if(cutVar==0){
      DF.sub2 <- DF.sub %>% dplyr::mutate(!!stratVariable.un := ifelse(!!predictor.un > cutVar,StratLevel1,StratLevel2)) %>%
        dplyr::mutate(!!stratVariable:=factor(!!stratVariable.un)) %>% dplyr::filter(complete.cases(!!event.un,!!endopoint.un))
...
fit_all <- function(selectedcancer,tcgaDF,survHRdf,cutPointsTable,covName,
  ...
  # selectedcancer <- datasetsEvent[2]
  # tcgaDF <- dataDF
  # survHRdf <-survival_HR_logrankP2
  # cutPointsTable = NULL
  # covName = mainVar
...
performSurvivalCalcs <- function(datasetsEvent, dataDF,mainVar, filterVar, survivalVar, sampleVar,covName,covType="cont",
  ...
  # varSelection, DF,var, varSelected, survType, sampleVar=sampleID,covName=varPlotName,covType="cont",stratifyMethod="median",sLevelA = "high",sLevelB = "low",onlyForest=TRUE
  # ##########################
...
runMultiSurvival_aPD1 <- function(DF,var,varSelection,varSelected,survType,sampleID,varPlotName,printP=FALSE,saveP = FALSE,savePath,onlySign=TRUE,selectFacets=NULL){
