...
subtitle: <center> Post-processing predictive modeling results for Scheme 3 (melanoma specific)</center>
...
unlink("scripts/main/E_06_aPD1_RFE_multivariate_postProcessing_Scheme3_cache", recursive = TRUE)
```
...
# SCHEME 3: MELANOMA specific

```{r s3start,warning=F, message=F,echo=TRUE, eval=TRUE, fig.width=10, fig.height=12}
######################
### Model variables ###
######################
...
immdata.TcrBcr.BLAD.MEL.ready <-immdata.TcrBcr.full.red.sub.ALL.filt %>% dplyr::filter(tissue %in% c("melanoma") ) %>% droplevels() # All melanoma & bladder, PD, CR, PR, complete data
immdata.TcrBcr.ALL.ready <- immdata.TcrBcr.full.red.sub.filt %>% dplyr::filter(tissue %in% c("melanoma") ) %>% droplevels() # All tissues, PD, CR, PR, complete data
...
```{r s3rfeOpt,warning=F, message=F,echo=TRUE, eval=TRUE,results='asis', fig.width=10, fig.height=12}
#######################
## RUN SINGLE MODELS ##
#######################
...
```{r s3loadFinalopt,warning=F, message=F,echo=TRUE, eval=TRUE, fig.width=10, fig.height=12}

optList <-loadRData(paste0(modelsPath,"paramOptim/","model_FINALoptParams",modelTypeA,modelTypeB, modelTypeC,Rdata.suffix))
```

```{r s3nonRFEmodels,warning=F, message=F,echo=FALSE, eval=TRUE, fig.width=10, fig.height=12}
if(isTRUE(run_clean)){
  ## SOLO MODELS
  res.tcr.rich <-trainRUN(immdata.TcrBcr.BLAD.MEL.ready,split = optList$split,ClassLevels = c("CRPR","PD"),PositiveClass = "CRPR",LevelIndex = 1,
           ...
           positiveCoefs=FALSE, filter=TRUE,filterVar="dataset",filterSelected=c("Gide","Rod","Hugo","Riaz","Liu"),
...
## RFE-`r optimal.model` vs Univariate & Bi-/Tri-variate LOGREG MODELS

Moving forward with the RFE-`r optimal.model`, since it seems to be the best performing model.

```{r s3rfeVSlogreg,warning=F, message=F,echo=TRUE, eval=TRUE,results='asis', fig.width=6, fig.height=6}
## SELECTING MODELS
...
rfeMods <- names(train.rfe)
rfeMods.n <-sapply(rfeMods, function(x) strsplit(x,":")[[1]][1])
names(rfeMods.n) <- NULL
rfeMods.n <- str_replace(rfeMods.n,"-"," ")
idxBestRFE <-which(rfeMods.n==optimal.model)

rfeBestfeats <- train.rfe[[idxBestRFE]]$optVariables
rfeBestfeats <- ifelse(rfeBestfeats=="BCR.IGHrichness.ds","BCR richness",
                       ifelse(rfeBestfeats=="TIS_sigscore","TIS",
                              ifelse(rfeBestfeats=="Bcells","B cells",
                                     ifelse(rfeBestfeats=="CD8cells","CD8 cells",
                                            ifelse(rfeBestfeats=="CD4cells","CD4 cells",
                                                   ifelse(rfeBestfeats=="TumorPurity","Tumor Purity",
                                                          ifelse(rfeBestfeats=="TCRArichness.ds","TCR richness",rfeBestfeats)))))))
models.final <- c(train.modelsSolo,
                  train.modelsBivariate,
                  train.modelsTrivariate,
                  train.modelsQuadravariate,
                  train.modelsALL,
                  list(nbModel =train.rfe[[idxBestRFE]]))
...
names(models.final) <- c('M01: R ~ TCRa richness','M02: R ~ BCRigh richness', 
                      ...
                      "M31 (LR): R ~ TCR richness + BCR richness + logTMB + PD-L1 + TIS GEP",
                         paste0("M32 (RFE-",optimal.model,"): R ~ ",paste(rfeBestfeats,collapse = " + ")))
sigs <- sigs <- c("TCRArichness.ds", "logTMB","PDL1","BCR.IGHrichness.ds","TIS_sigscore",
...
```{r s3rfeComp,warning=F, message=F,echo=TRUE, eval=TRUE,results='asis', fig.width=20, fig.height=20}
if(isTRUE(run_clean)){
...
writeLines(capture.output(session_info), paste0(sessionInfoPath,"E_06_aPD1_RFE_multivariate_postProcessing_Scheme3.txt"))
