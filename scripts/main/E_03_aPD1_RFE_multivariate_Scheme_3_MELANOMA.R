...
# Rscript ./E_03_aPD1_RFE_multivariate_Scheme_3_MELANOMA RF
...
immdata.TcrBcr.BLAD.MEL.ready <-immdata.TcrBcr.full.red.sub.ALL.filt %>% filter(tissue %in% c("melanoma") ) %>% droplevels() # All melanoma & bladder, PD, CR, PR, complete data
immdata.TcrBcr.ALL.ready <- immdata.TcrBcr.full.red.sub.filt %>% filter(tissue %in% c("melanoma") ) %>% droplevels() # All tissues, PD, CR, PR, complete data
...
if (args[1]=="NB") {
  ...
  ref_rf_res<- apply(myparamGrid1, 1,function(x) fullOptimalCutoffModelResult.RFE(df =  immdata.TcrBcr.BLAD.MEL.ready ,
                                                                                  ...
                                                                                  filt = TRUE,filtVar = "dataset",filtSel = c("Gide","Rod","Hugo","Riaz","Liu"),
  ...
  model_rf <-trainRUN.RFE(DF = immdata.TcrBcr.BLAD.MEL.ready,
                          ...
                          positiveCoefs=FALSE,
                          filter=TRUE,filterVar="dataset",filterSelected=c("Gide","Rod","Hugo","Riaz","Liu"),
