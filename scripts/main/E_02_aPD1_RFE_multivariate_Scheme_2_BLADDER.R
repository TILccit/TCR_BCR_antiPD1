...
if (length(args)==0) {
  stop("At least one argument must be supplied: model algorithm. Options: LR, NB, RF, KNN, SVMr, SVMl", call.=FALSE)
} 

## HOW TO RUN SCRIPT FROM COMMAND LINE IN COMPUTEROME #####
# module load intel/perflibs/2020_update4 gcc/11.1.0 R/4.1.0
# Rscript ./E_02_aPD1_RFE_multivariate_Scheme_2_BLADDER.R LR
...
# DF <-immdata.TcrBcr.full.red.sub.ALL # ONLY MELANOMA AND BLADDER
...
## INSTEAD OF FILTERINF FOR NO MISSING TMB, I DO THAT FOR ALL COVARIATES OF INTEREST
...
immdata.TcrBcr.BLAD.MEL.ready <-immdata.TcrBcr.full.red.sub.ALL.filt %>% filter(tissue %in% c("bladder") ) %>% droplevels() # All melanoma & bladder, PD, CR, PR, complete data
immdata.TcrBcr.ALL.ready <- immdata.TcrBcr.full.red.sub.filt %>% filter(tissue %in% c("bladder") ) %>% droplevels() # All tissues, PD, CR, PR, complete data
...
if (args[1]=="NB") {
  ...
  ref_rf_res<- apply(myparamGrid1, 1,function(x) fullOptimalCutoffModelResult.RFE(df =  immdata.TcrBcr.BLAD.MEL.ready ,
                                                                                  ...
                                                                                  filt = TRUE,filtVar = "dataset",filtSel = c("Powles","Mari"),
  ...
  # rfe_nb <- loadRData(paste0(modelsPathC,"optimizationParamsModels","_RFE","_NB",modelTypeA,modelTypeB, modelTypeC,Rdata.suffix))
  # rfe_nb[which(rfe_nb$ROC==max(rfe_nb$ROC)),]
  # hist(rfe_nb$ROC)
  # rfe_nb[order(rfe_nb$ROC, decreasing = TRUE),] %>% head()
  # rfe_nb.opt <-rfe_nb[which(rfe_nb$ROC==max(rfe_nb$ROC)),]
  ...
  # rfe.models.opt.DF.m <- rfe.models.opt.DF %>% select(model, ROC, PPV, NPV, split, kfolds, subsampling) %>% melt(id.vars = c("model","split", "kfolds", "subsampling"))
  # 
  ...
  #
  # # ggplot(rfe.models.opt.DF %>% select(model, ROC, PPV, NPV) %>% melt(), aes(fill=variable, y=value, x=model)) +
  ...
  model_rf <-trainRUN.RFE(DF = immdata.TcrBcr.BLAD.MEL.ready,
                          ...
                          positiveCoefs=FALSE,
                          filter=TRUE,filterVar="dataset",filterSelected=c("Powles","Mari"),
