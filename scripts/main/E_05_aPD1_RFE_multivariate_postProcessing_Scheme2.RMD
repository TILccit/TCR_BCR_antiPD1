...
subtitle: <center> Post-processing predictive modeling results for Scheme 2 (bladder specific)</center>
...
unlink("scripts/main/E_05_aPD1_RFE_multivariate_postProcessing_Scheme2_cache", recursive = TRUE)
```
...
# SCHEME 2: BLADDER specific

```{r s2start,warning=F, message=F,echo=TRUE, eval=TRUE, fig.width=10, fig.height=12}
######################
### Model variables ###
######################
...
immdata.TcrBcr.BLAD.MEL.ready <-immdata.TcrBcr.full.red.sub.ALL.filt %>% dplyr::filter(tissue %in% c("bladder") ) %>% droplevels() # All melanoma & bladder, PD, CR, PR, complete data
immdata.TcrBcr.ALL.ready <- immdata.TcrBcr.full.red.sub.filt %>% dplyr::filter(tissue %in% c("bladder") ) %>% droplevels() # All tissues, PD, CR, PR, complete data
```
...
```{r s2rfeOpt,warning=F, message=F,echo=TRUE, eval=TRUE,results='asis', fig.width=10, fig.height=12}
#######################
## RUN SINGLE MODELS ##
#######################
...
```{r s2loadFinalopt,warning=F, message=F,echo=TRUE, eval=TRUE, fig.width=10, fig.height=12}

optList <-loadRData(paste0(modelsPath,"paramOptim/","model_FINALoptParams",modelTypeA,modelTypeB, modelTypeC,Rdata.suffix))
```

```{r s2nonRFEmodels,warning=F, message=F,echo=FALSE, eval=TRUE, fig.width=10, fig.height=12}
if(isTRUE(run_clean)){
  ## SOLO MODELS
  res.tcr.rich <-trainRUN(immdata.TcrBcr.BLAD.MEL.ready,split = optList$split,ClassLevels = c("CRPR","PD"),PositiveClass = "CRPR",LevelIndex = 1,
           ...
           positiveCoefs=FALSE, filter=TRUE,filterVar="dataset",filterSelected=c("Powles","Mari"),
...
```{r s2rfeVSlogreg,warning=F, message=F,echo=TRUE, eval=TRUE,results='asis', fig.width=6, fig.height=6}
## SELECTING MODELS
...
```{r s2rfeComp,warning=F, message=F,echo=TRUE, eval=TRUE,results='asis', fig.width=20, fig.height=20}
if(isTRUE(run_clean)){
...
writeLines(capture.output(session_info), paste0(sessionInfoPath,"E_05_aPD1_RFE_multivariate_postProcessing_Scheme2.txt"))
